...
@app.route("/")
def main():
    # First see if we're authorized
    if(dash.apiAuthorized() == False)
        return redirect("/auth/")

    


    if(dashboard.getHostUrl() == None):
        print "Trying to set Host URL"
        dashboard.setHostUrl()
    if(dashboard.getHostUrl() == None):
        print "Error setting Host URL"
        exit()
    if(dashboard.initd()):
        return render_template('main.html', title='SmartThings PyDash')
    else:
        return redirect("/auth/")
...
def auth():
    print "auth.."
    redirecturl = dashboard.authInit("http://"+ dashboard.getHostUrl() + "/callback")
    return redirect(redirecturl)

@app.route("/reauth/")
def reauth():
    print "auth.."
    redirecturl = dashboard.reauth("http://"+ dashboard.getHostUrl() + "/callback")
    return redirect(redirecturl)

@app.route("/callback/")
def callback():
    authcode = request.args.get('code', '')
    redirecturl = dashboard.authSecond(authcode,"http://"+ dashboard.getHostUrl() + "/callback")
    return redirect(redirecturl)
...
def switchState(path):
    if request.method == 'POST':
        ...
        dashboard.toggleSwitch(device)
        status = dashboard.getDeviceStatus('switch',device,'state')
        if (status == 'on'):
            status = 'off'
        else:
            status = 'on'
...
def selectedHueSet(path):
    device =  unquote(path)
    if request.method == 'POST':
        hue=dashboard.setSelectedHue(device)
        results = {'Set':True}
        return Response(json.dumps(results))
    else:
        hue=dashboard.setSelectedHue(device)
...
def selectedDimmerSet(path):
    device =  unquote(path)
    if request.method == 'POST':
        hue=dashboard.setSelectedDimmer(device)
        results = {'Set':True}
        return Response(json.dumps(results))
    else:
        #device =  unquote(path)
...
def setColor():
    if request.method == 'POST':
        data =  request.form
        device = dashboard.getSelectedHue()
        color = data['color']
        hue = data['hue']
        sat = data['sat']
        results = dashboard.setColorHSLA(device,hue,sat,color)
        return Response(json.dumps(results))
    else:
        results = {'Set':True}
        return Response(json.dumps(results))
...
def setDimmer():
    if request.method == 'POST':
        data =  request.form
        device = dashboard.getSelectedDimmer()
        level = data['level']
        if(level == "off"):
            dashboard.setSwitch(device,'off')
        else:
            dashboard.setSwitch(device,'on')
            dashboard.setDimmer(device,level)
        results = {'Set':True}
        return Response(json.dumps(results))
    else:
        results = {'Set':True}
...
def selectedDimmer():
    if request.method == 'POST':
        ...
        hue=dashboard.getSelectedDimmer()
...
def colorValue(path):
    if request.method == 'POST':
        ...
        device =  unquote(path)
        hue = dashboard.getDeviceStatus('color',device,'hue')
        sat = dashboard.getDeviceStatus('color',device,'sat')
...
def contactState(path):
    if request.method == 'POST':
        ...
        device =  unquote(path)
        status = dashboard.getDeviceStatus('contact',device,'state')
...
def humidityValue(path):
    if request.method == 'POST':
        ...
        device =  unquote(path)
        status = dashboard.getDeviceStatus('humidity',device,'value')
...
def powerState(path):
    if request.method == 'POST':
        ...
        device =  unquote(path)
        power = dashboard.getDeviceStatus('power',device,'value')
        energy  = dashboard.getDeviceStatus('power',device,'energy')
...
def presenceState(path):
    if request.method == 'POST':
        ...
        device =  unquote(path)
        status = dashboard.getDeviceStatus('presence',device,'state')
        results = {'state':status}
        return Response(json.dumps(results))
...
def motionState(path):
    if request.method == 'POST':
        ...
        device =  unquote(path)
        status = dashboard.getDeviceStatus('motion',device,'state')
        results = {'state':status}
        return Response(json.dumps(results))
...
def dimmerState(path):
    if request.method == 'POST':
        ...
        device =  unquote(path)
        state = dashboard.getDeviceStatus('dimmer',device,'state')
        level = dashboard.getDeviceStatus('dimmer',device,'level')
...
def tempState(path):
    if request.method == 'POST':
        ...
        device =  unquote(path)
        temp = dashboard.getDeviceStatus('temperature',device,'value')
        results = {'value':temp}
        return Response(json.dumps(results))
...
def run_st_pydash_app():
    import SocketServer
    SocketServer.BaseServer.handle_error = close_stream
    SocketServer.ThreadingTCPServer.allow_reuse_address = True
    st_pydash_app.run(app, xyzzy)


if __name__ == "__main__":
    api = smartthings_api.SmartThings()
